...
if [ "$reset_db" -eq 1 ]; then
  echo "${ORANGE}\xE2\x98\x85 Resetting Video Conference Example database ...${NOCOLOR}"
  # remove docker volumes (database data, etc.) and restart services
  docker-compose -f dev.docker-compose.yml down -v
else
  # restart services without removing volumes
  echo "${ORANGE}\xE2\x98\x85 Restarting Video Conference Example ...${NOCOLOR}"
...
echo "${ORANGE}\xE2\x98\x85 Waiting for Django to start ...${NOCOLOR}"
...
if [ "$reset_db" -eq 1 ]; then
  echo "${ORANGE}\xE2\x98\x85 Inserting initial data...${NOCOLOR}"
